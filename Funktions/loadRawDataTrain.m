function tempData = loadRawDataTrain(idx_start ,idx_end)
...
flag_event_A = 0;
flag_event_B = 0;
start_Time = datenum('20-Oct-2011 12:00:00.000');
end_Time = datenum('24-Oct-2011 21:00:00.000');
...
%        datestr(endTime)
     ...
     if ~isempty (intersect(sequanceTime,eventsTimeVecPhaseA))
         if size(intersect(sequanceTime,eventsTimeVecPhaseA),1)== 1 %just one event in every sequance
     ...
     if ~isempty (intersect(sequanceTime,eventsTimeVecPhaseB))
         if size(intersect(sequanceTime,eventsTimeVecPhaseB),1)== 1
